...
[ -n "$kali_repository_uri" ] || kali_repository_uri="https://deb.kali.org"
...
if [ "${TEMPLATE_FLAVOR}" = "kali-standard" ]; then
   aptInstall kali-linux
elif [ "${TEMPLATE_FLAVOR}" = "kali-full" ]; then
   aptInstall kali-linux-full
else
   error "TEMPLATE_FLAVOR is neither kali-standard nor kali-full, it is: ${TEMPLATE_FLAVOR}"
fi
